...
class capybara_dp():
    ...
    def aging_cb(dev_id, pipe_id, direction, parser_id, entry):
        smac = bfrt.capybara_dp.pipe.Ingress.smac
        dmac = bfrt.capybara_dp.pipe.Ingress.dmac
    ...
    def learning_migration(dev_id, pipe_id, direction, parser_id, session, msg):
        # migrate_request = bfrt.capybara_dp.pipe.Ingress.migrate_request
        # migrate_reply = bfrt.capybara_dp.pipe.Ingress.migrate_reply
...
### Setup L2 learning
sl2 = capybara_dp(default_ttl=10000)
sl2.setup()
set_bcast([24, 32, 36])
bfrt.complete_operations()
