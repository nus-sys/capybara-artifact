    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    apply {
        ...
        if(eg_intr_md.egress_port == PIPE_0_RECIRC){
            ...
            // FIG8: 2-server static target = the other node9 instance (10000 <-> 10001).
            // (min-rps selection is poisoned by dead register banks when only 2 of the
            //  12 registered backends are alive.)
            hdr.ipv4.dst_ip = 0x0A000109;
            hdr.udp.dst_port = hdr.tcpmig.origin_port ^ 1;
        }
        // tbl_individual_rps.apply();
    }
}
