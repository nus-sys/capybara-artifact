...
class main_eval():
    ...
    def __init__(self, default_ttl=60000):
        self.p4 = bfrt.main_eval_fig10.pipe
    ...
    def aging_cb(dev_id, pipe_id, direction, parser_id, entry):
        smac = bfrt.main_eval_fig10.pipe.Ingress.smac
        dmac = bfrt.main_eval_fig10.pipe.Ingress.dmac
    ...
    def learning_migration(dev_id, pipe_id, direction, parser_id, session, msg):
        # migrate_request = bfrt.main_eval_fig10.pipe.Ingress.migrate_request
        # migrate_reply = bfrt.main_eval_fig10.pipe.Ingress.migrate_reply
...
# FIG10: static binding — client src_port p -> backend p%12 (client ports ~4080-4319)
for p in range(3900, 6300):
...
# Config B: two server nodes, six instances each. Group i lives on node9 for even
# i and node10 for odd i, at port 10000 + i//2.
for i in range(12):
    node = '10.0.1.9' if i % 2 == 0 else '10.0.1.10'
    p4.Ingress.backend_ip.mod(REGISTER_INDEX=i, f1=IPAddress(node))
    p4.Ingress.backend_port.mod(REGISTER_INDEX=i, f1=10000 + (i // 2))
