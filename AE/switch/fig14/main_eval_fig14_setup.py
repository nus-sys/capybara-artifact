...
class main_eval():
    ...
    def __init__(self, default_ttl=60000):
        self.p4 = bfrt.main_eval_fig8.pipe
    ...
    def aging_cb(dev_id, pipe_id, direction, parser_id, entry):
        smac = bfrt.main_eval_fig8.pipe.Ingress.smac
        dmac = bfrt.main_eval_fig8.pipe.Ingress.dmac
    ...
    def learning_migration(dev_id, pipe_id, direction, parser_id, session, msg):
        # migrate_request = bfrt.main_eval_fig8.pipe.Ingress.migrate_request
        # migrate_reply = bfrt.main_eval_fig8.pipe.Ingress.migrate_reply
...
for i in range(int(num_backends/3)):
    bfrt.pre.node.entry(MULTICAST_NODE_ID=10000 + i, MULTICAST_RID=10000 + i,
        MULTICAST_LAG_ID=[], DEV_PORT=[36, 24]).push()
...
# FIG8: static parity binding — client src_port p -> backend p%2 (client ports 400-511)
for p in range(0, 1024):
    p4.Ingress.reg_be_idx.mod(REGISTER_INDEX=p, f1 = p % 2)
...
# FIG8: 2 backends on node9 (ports 10000/10001), alternated across all 12 RR slots
# Two backends, one per node, both on the frontend port: this experiment migrates
# a connection between hosts rather than between processes on one host.
for i in range(12):
    p4.Ingress.backend_ip.mod(REGISTER_INDEX=i,
                              f1=IPAddress('10.0.1.8' if i % 2 == 0 else '10.0.1.9'))
