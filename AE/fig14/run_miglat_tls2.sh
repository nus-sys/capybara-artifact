...
ssh node8 "sudo pkill -INT -x https.elf 2>/dev/null; sleep 1; sudo pkill -x https.elf 2>/dev/null; for p in 0 1; do tmux kill-session -t tls\$p 2>/dev/null; done; sudo pkill -x dpdk-ctrl.elf 2>/dev/null; tmux kill-session -t dc8 2>/dev/null; true" >/dev/null 2>&1
sleep 2
ssh node8 "tmux new-session -d -s dc8 \"cd $T && make PREFIX=/homes/inho dpdk-ctrl-node8 > /tmp/ae-dc8.log 2>&1\"" >/dev/null 2>&1
sleep 14
ssh node8 "cd $T; for p in 0 1; do c=\$((p+1)); tmux new-session -d -s tls\$p \"cd $T && sudo -E env $POLICY CONFIGURED_STATE_SIZE=$SZ CAPY_LOG=all CORE_ID=\$c CONFIG_PATH=scripts/config/node8_config.yaml MTU=9000 MSS=9000 NUM_CORES=4 USE_JUMBO=1 LIBOS=catnip DATA_SIZE=256 LD_LIBRARY_PATH=\\/homes/inho/lib:\\/homes/inho/lib/x86_64-linux-gnu numactl -m0 bin/examples/rust/https.elf 10.0.1.8:1000\$p migrate > $D/$ID.be\$p 2>&1\"; sleep 2; done" >/dev/null 2>&1
sleep 4
NSRV=$(ssh node8 "pgrep -c https.el" 2>/dev/null)
...
rm -f $D/$ID.client
tmux new-session -d -s tlsc "sudo timeout ${TMO:-60} env OPENSSL_CONF=/tmp/ae-tls12.cnf numactl -m0 /homes/inho/Capybara/capybara-redis/src/redis-benchmark $TLSARGS -h 10.0.1.8 -p 10000 -t get -n $NREQ -c $CONNS --threads $CONNS > $D/$ID.client 2>&1"
sleep ${RT:-15}
tmux kill-session -t tlsc 2>/dev/null; sudo pkill -x redis-benchmark 2>/dev/null
sleep 1

# 3) graceful stop so the time logs are flushed
ssh node8 "sudo pkill -INT -x https.elf" >/dev/null 2>&1
sleep 5
ssh node8 "sudo pkill -x https.elf 2>/dev/null; true" >/dev/null 2>&1
