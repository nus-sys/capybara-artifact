#!/bin/bash
# Fig11 closed-loop panel, L7 Proxy (proxy-server relay; prism switch program just forwards): one backend count, a wrk
...
# usage: run_fig11_proxy_closed.sh <NB> "<conn list>" [dur_s]
# output: appends "RES proxy closed NB=<n> c=<c> rps=<r> gbps=<g> p99=<l>" lines
#         to ~/capybara-AE-runs/fig11/results_closed_proxy.txt
set -u
NB=$1; CONNS=${2:-"30 50 70"}; DUR=${3:-10}
T=/homes/inho/Capybara/capybara-fig11
OUT=~/capybara-AE-runs/fig11/results_closed_proxy.txt
DS=131072
...
start_servers () {
  ...
  ssh node8 "tmux kill-session -t fe 2>/dev/null; tmux new-session -d -s fe \"cd $T && sudo -E env CORE_ID=1 NUM_BE=$NB CONFIG_PATH=$T/scripts/config/node8_config.yaml MTU=9000 MSS=8960 NUM_CORES=4 USE_JUMBO=1 LIBOS=catnip DATA_SIZE=$DS LD_LIBRARY_PATH=\\/homes/inho/lib:\\/homes/inho/lib/x86_64-linux-gnu numactl -m0 timeout 900 $T/bin/examples/rust/proxy-server-fe.elf 10.0.1.8:10000 > /tmp/ae-cpfe.log 2>&1\"" >/dev/null 2>&1
  sleep 2
  for j in $(seq 0 $((NB-1))); do
    ssh node9 "tmux kill-session -t be$j 2>/dev/null; tmux new-session -d -s be$j \"cd $T && sudo -E env CORE_ID=$((j+1)) NUM_BE=$NB CONFIG_PATH=$T/scripts/config/node9_config.yaml MTU=9000 MSS=8960 NUM_CORES=4 USE_JUMBO=1 LIBOS=catnip DATA_SIZE=$DS LD_LIBRARY_PATH=\\/homes/inho/lib:\\/homes/inho/lib/x86_64-linux-gnu numactl -m0 timeout 900 $T/bin/examples/rust/proxy-server-be.elf 10.0.1.9:1000$j 10.0.1.8:10000 > /tmp/ae-cpbe$j.log 2>&1\"" >/dev/null 2>&1
  done
  sleep 5
}

for C in $CONNS; do
  # up to 3 full bring-up attempts (the proxy FE can miss the first try during
  # the switch link-flap window); only SANITY-FAIL after all attempts fail
  CODE=000
  for attempt in 1 2 3; do
    start_servers
    sleep $((attempt * 2))
    sudo ip route replace 10.0.1.8/32 dev ens85f1np1 advmss 8960 2>/dev/null
    CODE=$(timeout 8 curl -s -o /dev/null -w "%{http_code}" http://10.0.1.8:10000/get)
    [ "$CODE" = "200" ] && break
  done
  if [ "$CODE" != "200" ]; then
    echo "RES proxy closed NB=$NB c=$C SANITY-FAIL" | tee -a $OUT
    continue
  fi
...
PYEOF
  ...
  echo "RES proxy closed NB=$NB c=$C rps=$RPS gbps=$GBPS p99=$P99 errlines=$ERR" | tee -a $OUT
  sleep 2
done
