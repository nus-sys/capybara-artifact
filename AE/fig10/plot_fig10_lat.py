#!/usr/bin/env python3
"""Fig-10 reproduction. Peak throughput = highest offered load whose p99 is still
sane (<= 1 ms), which is how the paper's spreadsheets picked their operating
points: rows with a latency blow-up were discarded, rows the client could not
fully drive were kept."""
import os, sys, re
import matplotlib
matplotlib.use('Agg')
...
RES = sys.argv[1] if len(sys.argv) > 1 else os.path.expanduser('~/capybara-AE-runs/fig10/fig10_results_lat.txt')
P99_LIMIT = 1000.0
SZ = [1024, 4096, 8192]
...
for line in open(RES):
    ...
    if not m:
        continue
    cond, sz, _, ach, g, p99 = m.group(1), int(m.group(2)), m.group(3), int(m.group(4)), float(m.group(5)), float(m.group(6))
    if p99 > P99_LIMIT:
        continue
...
fig.suptitle('Fig. 10 reproduction — 12 servers, 720 conns, Zipf-1.2 (3 clients, jumbo frames; peak = highest load with p99 <= 1 ms)',
             fontsize=8.5, y=1.0)
fig.tight_layout()
...
for cond in ['LWRR', 'Capybara', 'Uniform']:
    n = [f'{100*o/u:.0f}%' if u else 'n/a' for o, u in zip(OURS[cond], OURS['Uniform'])]
    print(f'{cond:9s}: {[round(v,1) for v in OURS[cond]]} Gbps  ({n} of ideal)')
